<script lang="ts">
	...
	const { children, email = null }: Props = $props();
...
<Dialog.Root open={true}>
	...
	<Dialog.Content class="lg:min-w-250 overflow-hidden bg-background">
		<div class="relative pt-4 px-5 sm:px-6 lg:px-8">
			...
			<div class="absolute bottom-16 right-12 text-muted-foreground/80 text-right p-2 text-xs">
				{#if email}
			...
			<Dialog.Header>
				...
				<Dialog.Title
					class="font-serif font-semibold text-2xl sm:text-3xl lg:text-4xl leading-tight"
				>
					Plant a few seeds in the moneypot
				</Dialog.Title>
				<Dialog.Description class="text-sm sm:text-base text-muted-foreground">
					Every <span class="font-semibold text-foreground">{sym}1</span> grows{' '}
					<span class="font-semibold text-foreground">{EUR_TO_PRIVATE_SEEDS} seeds 🌱</span> for you
					and{' '}
					<span class="font-semibold text-foreground">{EUR_TO_PUBLIC_SEEDS} seeds 🌱</span> shared with
					free users.
				</Dialog.Description>
			</Dialog.Header>

			<div class="mt-8 mb-8 grid gap-5 lg:grid-cols-2 lg:gap-6">
				<div>
			...
			<div class="mb-4 grid gap-5 lg:grid-cols-2 lg:gap-6">
				<div class="p-4 sm:p-5">
					<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
						<div>
							...
							<div class="mt-1 text-xs text-muted-foreground">
								{(frequency === 'year' &&
									...
									''}
								{(frequency === 'month' && 'rolls over for 6 months') || ''}
								{(frequency === 'once' && 'yours to spend, kept 1 year') || ''}
							</div>
						</div>
						<div class="text-left sm:text-right">
							...
							<div class="mt-1 text-xs text-muted-foreground">shared with all users</div>
						</div>
					</div>
					...
					<div class="mt-4 rounded-2xl bg-accent/50 backdrop-blur p-3 sm:p-4">
						...
						<ul class="relative text-xs sm:text-sm">
							...
							{#each FEATURE_COSTS as f (f.name)}
								...
								<li
									class={cn('flex items-center gap-2 py-1.5', f.comingSoon && 'opacity-60 blur-xs')}
								>
				...
				<div
					class="p-4 sm:p-5 h-min bg-card rounded-xl rotate-2 mx-8 shadow-(--shadow-soft) my-auto"
				>
					<div
			...
			{:else}
				...
				>
					Don't need seeds? Support development directly on GitHub Sponsors <ExternalLink
						class="size-2.5"
					/>
				</Button>
			{/if}
