...
/**
 * Fills the NOT NULL recipe columns with safe defaults. The LLM repair path can
 * drop individual fields (e.g. when the model returns `null`), and that output
 * may also already be cached — so every write goes through this normalizer.
 */
function recipeNotNullFields(recipe: EnrichedRecipeOutput['recipe']): RecipeNotNullFields {
	return {
...
export async function saveEnrichedRecipe(
	...
	const fields = recipeNotNullFields(enrichedRecipe.recipe);

	const title = fields.title.length >= 47 ? `${fields.title.slice(0, 47)}...` : fields.title;
	...
	const { data: enrichedInsertData, error: enrichedInsertError } = await supabase
		...
		.update({
			...
			cleanup_level: fields.cleanup_level,
			cost_level: fields.cost_level,
...
export async function insertImportedRecipe(
	...
	const fields = recipeNotNullFields(input.enriched.recipe);
