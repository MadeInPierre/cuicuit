...
import type { PlanItemRow } from './list-items.js';
import type { PlanMealRow, ShoppingIngredient } from './list-meals.js';

// PURE helper (no defineOp): combines synced meal/item rows into a derived shopping
// list. Runs locally on synced rows under PowerSync (M5), hence dependency-free.
// Moved verbatim from `src/routes/(app)/shopping-list/generate-shopping-list.ts`
// (old path re-exports this module for compatibility).
//
// Type names mirror the legacy `features/plans/queries` aliases:
// `PlanMealRow` = `MealWithRecipeAndIngredients`, `PlanItemRow` = `ShoppingListItem`.
